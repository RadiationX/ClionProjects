function sumMatrix(A, B) {
    if (A.length != B.length || A[0].length != B[0].length)
        throw "Ошибка сложения: Не совпадают размеры матриц ["+A.length+", "+B.length+"]";
    var m = A.length,
        n = A[0].length,
        C = [];
    for (var i = 0; i < m; i++) {
...
function subMatrix(A, B) {
    if (A.length != B.length || A[0].length != B[0].length)
        throw "Ошибка вычитания: Не совпадают размеры матриц ["+A.length+", "+B.length+"]";
    var m = A.length,
        n = A[0].length,
        C = [];
    for (var i = 0; i < m; i++) {
...
function multiplyMatrix(A, B) {
    var rowsA = A.length,
        ...
        C = [];
    if (colsA != rowsB)
        throw "Ошибка умножения: Не совпадают столбцы A и строки B ["+colsA+", "+rowsB+"]";
